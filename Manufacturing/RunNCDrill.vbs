...
Sub SetupChartHoleSymbols(oNCDrillHoles)
    ...
    For Each oNCDrillHole In oNCDrillHoles
        ...
        oNCDrillHole.DrillSymbolSize(eengUnitMM) = dSymbolSize
...
Sub WriteSetupFile(oNCDrillEngine, isSchemeFile)
    ...
    End If
    If isSchemeFile = 1 Then
        For Each oNCDrillHole In oNCDrillHoles
            setupFile.WriteLine ("")
            setupFile.WriteLine (".HoleName  " & """" & oNCDrillHole.Name & """")
            setupFile.WriteLine ("  ..AssignmentMethod  kAutoGenerateDrlsym")
            setupFile.WriteLine ("  ..DrillSymbolType  kUndefinedDrlsym")
            setupFile.WriteLine ("  ..SymbolSize  " & dSymbolSize*1000000)
            setupFile.WriteLine ("  ..SymbolName  """"")
        Next
    End If
End Sub
